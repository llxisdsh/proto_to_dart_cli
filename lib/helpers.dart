...
ListType? getInferredType(dynamic d) {
  if (d.runtimeType == int) {
    ...
    if (DateTime.tryParse(d) != null) {
      return ListType.DateTime;
    }
...
bool isASTLiteralDateTime(Node? astNode) {
  if (astNode != null && astNode is LiteralNode) {
    ...
    if (literalNode.value != null) {
      return DateTime.tryParse(literalNode.value!) != null;
    }
  }
  return false;
}

bool isASTLiteralDateTimeList(Node? astNode) {
  if (astNode != null && astNode is LiteralNode) {
    final LiteralNode literalNode = astNode;
    if (literalNode.value != null && (literalNode.value as List).isNotEmpty) {
      return DateTime.tryParse(literalNode.value[0]!) != null;
    }
  }
  return false;
}
